...
total=9   # +1 step for tmux‑nobar helper
#
...
helper='
# --- nvim detached in tmux (no status bar) ---
nvim() {
  ...
  local session="${dir}_$(openssl rand -hex 3)"
  tmux -L "$sock" -f /tmp/tmux_nobar.conf new-session -s "$session" \
       bash -c "nvim \"$*\""
}
# --- end nvim helper ---
'
...
for rc in ~/.bashrc ~/.zshrc; do
  if [ -f "$rc" ]; then
    if ! grep -q "nvim() {" "$rc"; then
      ...
      tmp_helper=$(mktemp)
      printf "%s\n" "$helper" > "$tmp_helper"
      sed -i "/# --- nvim detached in tmux/,/# --- end nvim helper/ {
        r $tmp_helper
        d
      }" "$rc"
      rm "$tmp_helper"
